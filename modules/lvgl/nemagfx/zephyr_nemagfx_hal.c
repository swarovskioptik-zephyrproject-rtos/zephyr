...
int32_t nema_sys_init(void)
{
	...
	error_code = tsi_malloc_init_pool_aligned(0, (void *)nemagfx_pool_mem,
						  (uintptr_t)nemagfx_pool_mem,
						  NEMAGFX_MEM_POOL_SIZE, 1, 32);
...
void nema_buffer_flush(nema_buffer_t *bo)
{
	...
	sys_cache_data_flush_range((void *)bo->base_phys, bo->size);
...
int nema_mutex_lock(int mutex_id)
{
	...
	if ((mutex_id >= 0) && (mutex_id <= MUTEX_MAX)) {
		ret = (k_mutex_lock(&nema_mutexes[mutex_id], K_FOREVER) == 0) ? 0 : -1;
	}
